-- 1. Продажи по категориям за последний месяц
SELECT 
    c.category_name,
...
-- 4. Средняя продолжительность сессий и активность пользователей по дням недели
SELECT 
...
-- 5. Анализ рейтингов товаров по категориям
SELECT 
    c.category_name,
...
-- 8. Популярные категории по количеству просмотров (на основе сессий)
...
-- 9. Анализ времени от регистрации до первой покупки
SELECT 
